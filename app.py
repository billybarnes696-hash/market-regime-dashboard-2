...
def parse_ny_file(filepath):
    """
    Parses NYMO/NYHL CSVs.
    Handles formats like: "MM/DD/YYYY, Val, Val, Val, Val, 0"
    ...
    try:
        # Read raw lines to handle inconsistent formatting if necessary
        # But standard read_csv usually works if we skip bad lines
        df = pd.read_csv(filepath, header=None, names=['Date', 'V1', 'V2', 'V3', 'V4', 'Extra'], 
        ...
        # Force numeric conversion for all value columns
        ...
        # Determine if it's NYMO (single value repeated) or NYHL (distinct values)
        # Check first 10 valid rows
        sample = df.head(10)
        is_single_value = True
        if not sample.empty:
            # If V1, V2, V3, V4 are effectively identical, it's likely NYMO
        ...
        if is_single_value:
            ...
            # Average the 4 components for NYHL composite sentiment
...
if uploaded_nymo is not None:
    # Save temporarily to parse with existing function
...
if df_nymo is not None and df_nyhl is not None:
    ...
    if df_merged.empty:
        st.warning("No overlapping dates found between the two files.")
        st.stop()
    ...
    df_merged['Cum_NYMO'] = pd.to_numeric(df_merged['Cum_NYMO'], errors='coerce')
    df_merged['Cum_NYHL'] = pd.to_numeric(df_merged['Cum_NYHL'], errors='coerce')
